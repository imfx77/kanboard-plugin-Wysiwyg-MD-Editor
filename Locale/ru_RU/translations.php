...
* При нажатии на значки эмодзи вставляется и заменяется текущий выделенный текст.',
    'WysiwygMDEditor_SETTINGS_EMOJIFONT_ENABLE' => 'Использовать внутренний шрифт эмодзи',
    'WysiwygMDEditor_SETTINGS_EMOJIFONT_DESCR' => 'Плагин предоставляет явный шрифт эмодзи (на основе Mozilla Twemoji), если требуется переопределить существующие представления эмодзи.
* Его использование гарантирует, что эмодзи будут выглядеть одинаково во всех браузерах на всех платформах.
* Кроме того, ни один эмодзи не будет отсутствовать или отображаться неправильно из-за неполных или старых версий шрифтов браузера или системы.
* ⚠ С другой стороны, поскольку эта опция переопределяет цепочку приоритетов шрифтов по умолчанию, тексты на страницах базы знаний могут выглядеть по-другому !
* ⚠ При включении шрифт будет использоваться на всех обычных страницах, в режимах редактирования и рендеринга EasyMDE, независимо от настройки Emoji Picker !
* ⚠ Изменение этого параметра требует явной перезагрузки страницы для вступления изменений в силу !',
