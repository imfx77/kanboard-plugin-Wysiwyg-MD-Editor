const EmojiPicker = function(options) {
    ...
    let localDocument = this.options.localDocument ? this.options.localDocument : document;
    let categoriesHTML = '';
    let emojisHTML = '';
    let emojisList = undefined;
    let pickerOuterContainer = undefined;
    ...
    const functions = {
        ...
        styles: () => {
            ...
            const styles = `
                <style>
                    .fg-emoji-picker-container-outer {
                    ...
                    .fg-emoji-picker-container-outer-hidden {
                        display: none;
                    }
                    .fg-emoji-picker-container-middle {
                        display: table-cell;
                        vertical-align: middle;
                    }
                    .fg-emoji-picker-container {
                        margin: 0 auto;
                        position: relative;
                    ...
                    .fg-emoji-picker-nav {
                        background-color: #646772 !important;
                    }
                    .fg-emoji-picker-nav li svg {
                        transition: all .2s ease;
                        fill: white;
                    }
                    .fg-emoji-picker-nav li:hover svg {
                        fill: black;
                    }
                    .fg-emoji-picker-nav ul {
                        display: flex;
                        flex-wrap: wrap;
                        list-style: none;
                        margin: 0;
                    ...
                    .fg-emoji-picker-nav ul li:hover {
                        background-color: #e9ebf1 !important;
                    }
                    .fg-emoji-picker-nav ul li.active {
                        background-color: #e9ebf1 !important;
                    }
                    .fg-emoji-picker-nav ul li.emoji-picker-nav-active {
                        background-color: #e9ebf1 !important;
                    }
                    .fg-emoji-picker-nav ul li.emoji-picker-nav-active svg {
                        fill: #646772;
                    }

                    .fg-emoji-picker-move-button {
                        cursor: move !important;
                        -ms-touch-action: none !important;
                        touch-action: none !important;
                    }
                    .fg-emoji-picker-special-buttons {
                        background-color: ${this.options.specialButtons ? this.options.specialButtons : '#ed5e2877'} !important;
                    }
                    .fg-emoji-picker-special-buttons:last-child {
                    ...
                    .fg-emoji-picker-list {
                        ...
                        overflow-x: hidden;
                        height: 323px;
                        background-color: #f3f3f3aa !important;
                    }
                    .fg-emoji-picker-category-title {
                        ...
                        border-bottom: 1px solid #606c71 !important;
                    }
                    .fg-emoji-picker-icon-wrapper {
                        cursor: pointer;
                    ...
                    .fg-emoji-picker-icon-wrapper:hover {
            ...
            localDocument.head.insertAdjacentHTML('beforeend', styles);
        },

        render: (e) => {
            if (this.options.softClose && localDocument.querySelector('.fg-emoji-picker-container-outer')) {
                functions.showPicker(e);
                return;
            }

            // init insertion element
            const index = this.options.trigger.findIndex(item => item.selector === e.data.trigger);
            this.insertInto = this.options.trigger[index].insertInto;

            // construct emoji categories titles and icon wrapper sections (once only on first render)
            if (!emojisHTML.length) {
                for (const key in emojisConfig) {
                    if (emojisConfig.hasOwnProperty.call(emojisConfig, key)) {
                        const categoryObj = emojisConfig[key];

                        categoriesHTML += `<li title="${key}">${categoryIcons[key]}</li>`;

                        emojisHTML += `<div class="fg-emoji-picker-category-title" id="${key}">${key}</div>`;

                        emojisHTML += `<div>`;
                        categoryObj.forEach(ej => {
                            emojisHTML += `<p class="fg-emoji-picker-icon-wrapper" title="${ej.title}">${ej.emoji}</p>`;
                        });
                        emojisHTML += `</div>`;
                    }
                }
            ...
            // construct the emoji picker with containers
            const pickerHTML = `
                <div class="fg-emoji-picker-container-outer">
                <div class="fg-emoji-picker-container-middle">
                <div class="fg-emoji-picker-container">
                    <nav class="fg-emoji-picker-nav">
                        <ul>
                            <li class="fg-emoji-picker-special-buttons fg-emoji-picker-move-button" title="Move">${specialIcons.move}</li>
                            ${categoriesHTML}
                            ${this.options.closeButton ? `<li class="fg-emoji-picker-special-buttons fg-emoji-picker-close-button" title="Close">${specialIcons.close}</li>` : ''}
                        </ul>
                    </nav>

                    <div class="fg-emoji-picker-search">
                        <input type="text" placeholder="Search" class="fg-emoji-picker-search-input" ${this.options.searchAutofocus ? 'autofocus' : 'tabindex="-1"'}/>
                    </div>
            ...
            // (re)insert emoji picker element and init variables
            if (pickerOuterContainer) {
                jQuery(pickerOuterContainer).remove();
            }
            localDocument.body.insertAdjacentHTML('beforeend', pickerHTML);
            pickerOuterContainer = localDocument.querySelector('.fg-emoji-picker-container-outer');
            emojisList = undefined;

            // bind events
            if (this.options.softClose) {
                jQuery(localDocument.querySelector('.fg-emoji-picker-close-button')).click(functions.hidePicker);
                jQuery(pickerOuterContainer).click(functions.autoHidePicker);
            } else {
                jQuery(localDocument.querySelector('.fg-emoji-picker-close-button')).click(functions.closePicker);
                jQuery(pickerOuterContainer).click(functions.autoClosePicker);
            }
            jQuery(localDocument.querySelectorAll('.fg-emoji-picker-icon-wrapper')).click(functions.insert);
            jQuery(localDocument.querySelectorAll('.fg-emoji-picker-nav li')).click(functions.categoryNav);
            jQuery(localDocument.querySelector('.fg-emoji-picker-search-input')).on("input", functions.search);

            jQuery(localDocument.querySelector('.fg-emoji-picker-container')).draggable({handle: '.fg-emoji-picker-move-button'});

            // autofocus if configured
            if (this.options.searchAutofocus) {
                setTimeout(() => {
                    localDocument.querySelector('.fg-emoji-picker-search-input').focus();
                }, 500);
            }
        },
        ...
        showPicker: (e) => {
            e.preventDefault();
            jQuery(pickerOuterContainer).removeClass('fg-emoji-picker-container-outer-hidden');
        },

        hidePicker: (e) => {
            e.preventDefault();
            jQuery(pickerOuterContainer).addClass('fg-emoji-picker-container-outer-hidden');
        },
        ...
        autoHidePicker: (e) => {
            if (pickerOuterContainer && !e.target.closest('.fg-emoji-picker-container')) {
                functions.hidePicker(e);
            }
        },
        ...
        insert: (e) => {
            ...
            const myField = localDocument.querySelectorAll(this.insertInto);
            const myValue = emoji;

            // Check if selector is an array
            myField.forEach(myField => {
                if (localDocument.selection) {
                    sel = localDocument.selection.createRange();
                    sel.text = myValue;
        ...
        categoryNav: (e) => {
            ...
            const category = jQuery(e.target.closest('li'));

            // only handle category buttons
            if (category.hasClass('fg-emoji-picker-special-buttons')) return;

            const title = category.attr('title');
            const emojiBody = localDocument.querySelector('.fg-emoji-picker-list');
            const destination = localDocument.getElementById(title);

            jQuery(localDocument.querySelectorAll('.fg-emoji-picker-nav li')).removeClass('emoji-picker-nav-active');
            category.addClass('emoji-picker-nav-active');

            emojiBody.scrollTo({
                top: destination.offsetTop - destination.offsetHeight - emojiBody.offsetTop,
                left: 0,
                behavior: "smooth",
            });
        },
        ...
        search: (e) => {
            ...
            if (!emojisList) {
                // build once on the first search
                emojisList = Array.from(localDocument.querySelectorAll('.fg-emoji-picker-icon-wrapper'));
            }

            emojisList.filter(emoji => {
                if (!emoji.getAttribute('title').toLowerCase().match(searchValue)) {
    ...
    jQuery(() => {
        // inject styles
        functions.styles();
        // bind all trigger elements
        this.trigger.forEach((trigger) => {
            jQuery(localDocument.querySelector(trigger)).click({trigger: trigger}, functions.render);
        });
    });
}
